...
const createPendingContractorPayment = async (
  ...
  const paymentId = `cp_${Date.now()}_${contractorToPay.contractor_id.slice(-4)}`;
  ...
  try {
    console.log(`Creating pending contractor payment record ${paymentId}...`, payrollData);
    const payrollRef = doc(db, `businesses/${businessAddress}/Contractor_payroll/${paymentId}`);
    await setDoc(payrollRef, payrollData);
    console.log(`Pending contractor payment record ${paymentId} created.`);
    return paymentId;
  } catch (firestoreError: any) {
    console.error("Error storing PENDING contractor payment record:", firestoreError);
    throw new Error(`Failed to store pending payment record: ${firestoreError.message}`);
  }
};
...
const updateFinalContractorPayment = async (
  ...
  const db = getFirestore(app);
  const paymentRef = doc(db, `businesses/${businessAddress}/Contractor_payroll/${paymentDocId}`);
  const contractorRef = doc(db, `businesses/${businessAddress}/contractors/${contractorId}`);
  const timestamp = serverTimestamp();
  ...
  if (status === "Success") {
      const historyRef = doc(db, `businesses/${businessAddress}/payments/${paymentDocId}`);
       ...
       if (paymentDocSnap.exists()) {
          ...
          console.warn(`Could not find payment record ${paymentDocId} to create history entry.`);
       }
  }

  try {
    console.log(`Updating contractor payment ${paymentDocId} to status: ${status}`, paymentUpdateData);
    await batch.commit();
    console.log(`Payment ${paymentDocId} and related records updated successfully.`);
    return true;
  } catch (firestoreError: any) {
    console.error(`Error updating payment record ${paymentDocId} to ${status}:`, firestoreError);
    throw new Error(`DB Update Failed: ${firestoreError.message}. Please check records manually.`);
  }
};
...
export default function ContractorPage() {
  ...
  const [pendingPaymentDocId, setPendingPaymentDocId] = useState<string | null>(null);
      ...
      if (businessAddress) {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            setCompanyData(null);
            showErrorToast("Business profile not found. Please complete setup.");
          }
        } catch (error) {
    ...
    if (firebaseUser && businessAddress) {
        ...
        getDoc(userDocRef).then(docSnap => {
            ...
            if (registeredAddress !== currentAddress) {
                 showErrorToast("Connected wallet doesn't match registered account.");
                 setIsLoadingData(false);
                 setIsLoadingAccount(false);
                 auth.signOut();
                 router.push("/auth/login");
             ...
             const contractorsRef = collection(db, "businesses", businessAddress, "contractors");
  ...
  const handleAddContractor = async () => {
    if (!contractorName.trim() || !contractorEmail.trim() || !inviteContractorRole || !contractorPayment) {
      showErrorToast("Please fill all required fields (Name, Email, Role, Payment).");
      return;
    }
    ...
    if (contractors.some(c => c.contractor_email?.toLowerCase() === contractorEmail.toLowerCase())) {
       showErrorToast("A contractor with this email already exists.");
       return;
     }
    if (!companyData || !companyData.name) {
        showErrorToast("Business name not found. Please complete your business profile.");
        return;
    }
    ...
    try {
      const contractorRef = doc(db, "businesses", businessAddress, "contractors", contractor_id);
  ...
  const initiatePaymentProcess = async () => {
    if (!contractorToPay || !businessAddress) {
     ...
     if (contractorToPay.status === "Invited" || !contractorToPay.contractor_wallet) {
       showErrorToast(`${contractorToPay.contractor_name} hasn't connected their wallet or is not 'Active'.`);
       setShowConfirmation(false); // Close modal
      return;
     }
      const paymentAmount = Number(contractorToPay.payment);
    if (isNaN(paymentAmount) || paymentAmount <= 0) {
       showErrorToast(`Invalid payment amount for ${contractorToPay.contractor_name}.`);
       setShowConfirmation(false);
      return;
     }
      const recipientAddress = contractorToPay.contractor_wallet as Address;
     if (!recipientAddress || !recipientAddress.startsWith('0x') || recipientAddress.length !== 42) {
        showErrorToast(`Invalid wallet address for ${contractorToPay.contractor_name}.`);
        setShowConfirmation(false);
        return;
      }
     const gasLimitNum = Number(gasLimitInput);
    if (isNaN(gasLimitNum) || gasLimitNum <= 21000) {
        showErrorToast("Invalid Gas Limit. Please enter a reasonable value (e.g., 200000).");
     ...
     try {
        ...
        setPaymentStatusMessage("Please approve transaction in wallet...");
         console.log(`Executing transferByEmployer: To=${recipientAddress}, Amount=${parsedAmount} units, Gas=${gasLimitNum}`);
         ...
         if (pendingPaymentDocId) {
             try {
                  await updateFinalContractorPayment(businessAddress, pendingPaymentDocId, contractorToPay.contractor_id, "Failed", null, `Initiation Failed: ${errorMsg}`);
              } catch (updateError: any) {
                 console.error("Failed to mark pending record as failed:", updateError);
                 setPaymentStatusMessage(`Initiation Failed (${errorMsg}). DB Update also failed. Check records.`);
              }
         }
         ...
         resetTransfer();
         // Do NOT close confirmation automatically on initiation failure
          // setShowConfirmation(false);
     }
 };
  ...
  useEffect(() => {
      if (transferSuccess && transferTxHash && pendingPaymentDocId && contractorToPay) {
         console.log("Contractor Payment Transaction Successful! Hash:", transferTxHash);
          ...
          updateFinalContractorPayment(
             ...
             .catch((dbError) => {
                 ...
                 setPaymentStatusMessage(`Transaction successful, but DB update failed: ${dbError.message}`);
   ...
   }, [transferSuccess, transferTxHash, pendingPaymentDocId, contractorToPay, businessAddress, /* Removed redundant states like setContractorToPay etc */ ]);
  ...
  useEffect(() => {
     if (transferError && pendingPaymentDocId && contractorToPay) {
         ...
         console.error("Contractor Payment Transaction Failed:", transferWriteError);
         ...
         updateFinalContractorPayment(
             ...
             console.error("DB Update also failed after TX error:", dbError);
              setPaymentStatusMessage(`Transaction failed (${errorMsg}). DB update also failed.`);
         ...
         setIsProcessingPayment(false); // Stop processing
          ...
          // Keep confirmation modal open to show the error message
         // setShowConfirmation(false);
     }
 }, [transferError, transferWriteError, pendingPaymentDocId, contractorToPay, businessAddress, transferTxHash /* Include tx hash */]);


 // --- Other Handlers (Edit, Delete, Export, Filter, etc.) ---

  // When dropdown selection changes, find the contractor object
  useEffect(() => {
      if (paymentContractorEmail) {
          const selected = contractors.find(c => c.contractor_email === paymentContractorEmail);
           // Reset contractorToPay if selected is invalid or not payable
           if (!selected || selected.status === "Paid" || selected.status === "Invited" || !selected.contractor_wallet) {
   ...
   const handleConfirmPayment = () => {
      ...
      if (contractorToPay) {
         // Double-check validity just before showing modal
          ...
          if (contractorToPay.status === "Invited" || !contractorToPay.contractor_wallet) {
                showErrorToast(`${contractorToPay.contractor_name} has not connected their wallet.`);
                return;
            }
            const paymentAmount = Number(contractorToPay.payment);
             if (isNaN(paymentAmount) || paymentAmount <= 0) {
                 showErrorToast(`Invalid payment amount configured for ${contractorToPay.contractor_name}. Please edit the contractor.`);
                 return;
            }
  ...
  const handleExportList = async () => { /* ... keep existing logic ... */
    setIsExporting(true);
    try {
  ...
  const handleDeleteClick = (contractor) => {
   ...
   const handleEditSubmit = async () => {
     ...
     // --- Validation ---
    if (!editFormData.contractor_name.trim() || !editFormData.contractor_email.trim() || !editFormData.role) {
     ...
     const paymentNum = parseFloat(String(editFormData.payment)?.replace(/[^0-9.-]+/g, "") ?? '');
      if (isNaN(paymentNum) || paymentNum < 0) { // Allow 0 payment? Maybe should be > 0? Depends on use case.
     ...
     const lowerCaseEmail = editFormData.contractor_email.trim().toLowerCase();
      if (contractors.some(c => c.contractor_id !== editingContractorId && c.contractor_email?.toLowerCase() === lowerCaseEmail)) {
     ...
     try {
        const db = getFirestore(app);
        const contractorRef = doc(db, "businesses", businessAddress, "contractors", editingContractorId);
  ...
  const handleDeleteConfirm = async () => { /* ... keep existing logic ... */
     if (!contractorToDelete || !businessAddress) return;

    const db = getFirestore(app);
     const contractorRef = doc(db, "businesses", businessAddress, "contractors", contractorToDelete.contractor_id);
  ...
  // --- Helper Components & Data ---
    ...
    // ... (keep existing FilterDropdown logic)
     ...
     return (
         <div className="relative inline-block text-left" ref={dropdownRef}>
           <button
             type="button"
              className={`inline-flex justify-between items-center w-48 rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
   ...
   const ContractorSelect = ({ contractors, value, onChange, disabled }) => (
    <select
      className={`mt-1 block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md ${disabled ? 'bg-gray-100 opacity-70 cursor-not-allowed' : 'bg-white'}`}
       value={value}
    ...
    >
      ...
      {contractors
        .filter(c => c.status !== 'Invited' && c.status !== 'Inactive' && c.contractor_wallet) // Filter for active/connected
         .map((contractor) => (
          <option
          ...
          >
             {contractor.contractor_name}{contractor.status === 'Paid' ? ' (Paid)' : ''}
           </option>
       ))}
     </select>
   );
  ...
  const Skeleton = () => (
    <tr>{[...Array(7)].map((_, i) => <td key={i} className="px-6 py-4"><div className="h-4 bg-gray-200 rounded animate-pulse"></div></td>)}</tr>
  );

  const CONTRACTOR_ROLES = [ /* ... keep existing roles ... */
     "Freelancer",
     "Plumbing Repair",
     "Real Estate Agent",
     "Content Creator",
      ...
      filtered = filtered.filter(
        (c) =>
          ...
          c.role?.toLowerCase().includes(query) ||
           (c.contractor_wallet && c.contractor_wallet.toLowerCase().includes(query)) || // Check if wallet exists
    ...
    <div className="max-w-[1400px] mx-auto p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      {/* Header */}
       <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
         <div>
          <h1 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-1">Contractor Management</h1>
      ...
      {/* Tabs */}
       <div className="border-b border-gray-200 mb-6">
        <nav className="-mb-px flex gap-6 sm:gap-8 overflow-x-auto" aria-label="Tabs">
            ...
            (tab) => (
               <button
                key={tab}
                 ...
                 onClick={() => {
                     setActiveTab(tab);
                     // Reset payment form when switching to Pay tab? Optional.
                      // if (tab === 'PAY CONTRACTOR') {
                       // setPaymentContractorEmail('');
                        // setContractorToPay(null);
                       // setPaymentStatusMessage('');
                      //}
                 }}
                disabled={isLoadingAccount || isLoadingData && tab !== 'INVITE CONTRACTOR'} // Allow invite even if list is loading
               >
                 {tab}
              </button>
            )
      ...
      {(isLoadingAccount) ? (
                ...
                <div className="flex items-center space-x-3 w-full sm:w-auto">
                    {/* Role Filter */}
                     <FilterDropdown
                       ...
                       onSelect={(selectedRole) => setContractorRole(selectedRole === "All Roles" ? "" : selectedRole)}
                      ...
                      <thead className="bg-gray-50">
                       ...
                       <tbody className="bg-white divide-y divide-gray-200">
                         {isLoadingData ? (
                           ...
                           getFilteredContractorsForDisplay().map((contractor) => (
                             <tr key={contractor.contractor_id} className="hover:bg-gray-50 transition-colors">
                               ...
                               <td className="px-4 py-3 whitespace-nowrap">
                                     <span className="text-xs text-gray-500 font-mono">
                                         {contractor.contractor_wallet ? `${contractor.contractor_wallet.substring(0, 6)}...${contractor.contractor_wallet.substring(contractor.contractor_wallet.length - 4)}` : "Not Connected"}
                                    </span>
                                 </td>
                                <td className="px-4 py-3 whitespace-nowrap">
                               ...
                               <td className="px-4 py-3 whitespace-nowrap">
                                ...
                                <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium" ref={actionButtonRef}> {/* Ref needed for modal positioning */}
                                     <button
                          ...
                          <tr>
                              <td colSpan="7" className="text-center p-6 text-sm text-gray-500">
                                 {contractors.length === 0 ? "No contractors added yet." : "No contractors match your filters."}
                              </td>
                          </tr>
                         )}
                        </tbody>
          ...
          {/* --- INVITE CONTRACTOR TAB CONTENT --- */}
           {activeTab === "INVITE CONTRACTOR" && (
             <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200 max-w-2xl mx-auto">
                <h2 className="text-xl font-semibold text-gray-800 mb-2">Invite Contractor</h2>
               <p className="text-sm text-gray-500 mb-6">Fill in the details below to invite a new contractor via email.</p>
               ...
               <div className="space-y-4">
                   {/* Form Fields */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                       <div>
                         ...
                         <input id="inviteName" type="text" className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500" value={contractorName} onChange={e => setContractorName(e.target.value)} />
                       </div>
                       <div>
                           <label htmlFor="inviteEmail" className="block text-sm font-medium text-gray-700 mb-1">Email *</label>
                         <input id="inviteEmail" type="email" className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500" value={contractorEmail} onChange={e => setContractorEmail(e.target.value)} />
                       </div>
                    </div>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                         <div>
                 ...
                 <div className="flex justify-end gap-3 pt-4">
                   ...
                   <button type="button" className="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-medium hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" onClick={handleAddContractor}>Invite Contractor</button>
                   </div>
                </div>
             </div>
           )}
          ...
          {/* --- PAY CONTRACTOR TAB CONTENT --- */}
           {activeTab === "PAY CONTRACTOR" && (
             <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200 max-w-2xl mx-auto">
                <h2 className="text-xl font-semibold text-gray-800 mb-2">Pay Contractor</h2>
               <p className="text-sm text-gray-500 mb-6">Select an active contractor with a connected wallet to initiate payment.</p>
                    ...
                    {contractorToPay && ( // Only show amount if a valid contractor is selected
                         ...
                         <label htmlFor="payAmount" className="block text-sm font-medium text-gray-700 mb-1">Amount (USDC)</label>
                           <div className="relative">
                             ...
                             <input id="payAmount" type="text" className="w-full pl-7 pr-3 py-2 border border-gray-300 rounded-md bg-gray-50 cursor-not-allowed" value={formatCurrency(contractorToPay.payment)} readOnly disabled/>
                             </div>
                        </div>
                     )}
                   {/* Gas Limit Input */}
                     ...
                     {/* Static Confirmation Text */}
                       {!paymentStatusMessage && !transferError && (
                         <p className="text-sm text-gray-600 mb-4">
                           Pay <span className="font-medium">{contractorToPay.contractor_name}</span> the amount of{' '}
                             <span className="font-semibold">{formatCurrency(contractorToPay.payment)}</span> USDC?
                           </p>
                      )}

                      {/* Status/Error Message Area */}
                       {paymentStatusMessage && (
                         <div className={`text-sm mb-4 px-3 py-2 rounded-md w-full break-words ${transferError ? 'bg-red-50 border border-red-200 text-red-700' : 'bg-blue-50 border border-blue-200 text-blue-700'}`}>
                      ...
                      <div className="w-full mb-5 text-left">
                           ...
                           <input
                             ...
                             className={`w-full px-3 py-1.5 border rounded-md text-sm shadow-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 ${isProcessingPayment || transferLoading || transferError ? 'bg-gray-100 border-gray-300 cursor-not-allowed' : 'border-gray-300'}`}
                               ...
                               min="21000"
                               step="1000"
                             disabled={isProcessingPayment || transferLoading || transferError} // Disable on error too until explicitly cancelled/retried
                           />
                       </div>
                     ...
                     <div className="flex justify-center gap-3 w-full">
                      ...
                      >
                        ...
                        <button
                           type="button"
                            className={`px-4 py-2 rounded-md text-sm font-medium text-white flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors
                                         ${isProcessingPayment || transferLoading || transferError ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'}`}
                            onClick={initiatePaymentProcess} // Always calls the initiation
                           disabled={isProcessingPayment || transferLoading || transferError} // Disable if processing or error occurred
                        >
                          {(isProcessingPayment || transferLoading) ? (
                          ...
                          {isProcessingPayment || transferLoading ? paymentStatusMessage.includes("wallet") ? 'Waiting...' : 'Processing...' : 'Confirm & Pay'}
                         </button>
                      </div>
                   </div>
               </motion.div>
                   ...
                   <div className="flex justify-between items-center mb-5">
                    ...
                    <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                      <div>
                         ...
                         <input id="editName" type="text" value={editFormData.contractor_name} onChange={e => setEditFormData({...editFormData, contractor_name: e.target.value})} className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-indigo-500"/>
                       </div>
                        <div>
                            <label htmlFor="editEmail" className="block text-sm font-medium text-gray-700 mb-1">Email *</label>
                            <input id="editEmail" type="email" value={editFormData.contractor_email} onChange={e => setEditFormData({...editFormData, contractor_email: e.target.value})} className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-indigo-500"/>
                         </div>
                        <div>
                        ...
                        <div>
                           <label htmlFor="editPayment" className="block text-sm font-medium text-gray-700 mb-1">Payment (USDC) *</label>
                            <input id="editPayment" type="number" min="0" step="0.01" value={editFormData.payment} onChange={e => setEditFormData({...editFormData, payment: e.target.value})} className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-indigo-500"/>
                         </div>
                       <div>
                          <label htmlFor="editNote" className="block text-sm font-medium text-gray-700 mb-1">Note</label>
                          <textarea id="editNote" value={editFormData.invitation_note} onChange={e => setEditFormData({...editFormData, invitation_note: e.target.value})} rows={3} className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-indigo-500"></textarea>
                       </div>
                         {/* Display Wallet and Status Readonly */}
                        <div className="grid grid-cols-2 gap-4 pt-2">
                             <div>
                                <p className="text-sm font-medium text-gray-700 mb-1">Wallet Address</p>
                               <p className="text-xs text-gray-500 font-mono bg-gray-100 p-2 rounded break-all">{selectedContractor.contractor_wallet || 'Not Connected'}</p>
                           </div>
                             <div>
                                <p className="text-sm font-medium text-gray-700 mb-1">Status</p>
                               <p className={`text-sm font-medium px-2 py-1 rounded inline-block ${selectedContractor.status === 'Paid' ? 'bg-green-100 text-green-800' : selectedContractor.status === 'Active' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800'}`}>
                ...
                >
                  <div className="w-12 h-12 rounded-full bg-red-100 flex items-center justify-center mx-auto mb-4">
                    ...
                    <p className="text-sm text-gray-600 mb-6">
                        Are you sure you want to delete <span className="font-medium">{contractorToDelete.contractor_name}</span>? This action cannot be undone.
                     </p>
